...
def test_buscar_Gato():
    ...
    for nombre in nombres:
        for documento in documentos[nombre]:
            ...
            inv.agregar_mascota(Caballo(nombre, apellido, documento, raza, color, comidaFavorita))
...
def test_fuzzing_buscar_pez():
    ...
    nombresDueños = ['Alina Barragan', 'Bilal Gonzalez', 'Humberto Almeida', 'Araceli Alemany', 'Rafaela Rios']
    comidaFavoritas = ["zanahoria", "remolacha", "espinaca", "batavia"]
    ...
    for i in range(cantidad_mascotas):
        ...
        comidaFavorita = random.choice(comidaFavoritas)
        if i % 10 == 0:
            especificacion = Especificacion_Tienda_Mascotas()
            especificacion.agregar_parametros('nombre', nombre)
